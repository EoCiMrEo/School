...
import { validateMatchPreference, getAllowedRange } from '../../../utils/rankUtils';
...
const MatchmakingPreferences = ({ 
  ...
  const handleStartMatchmaking = () => {
    // Validate
    const validation = validateMatchPreference(currentUserElo, skillRange[0], skillRange[1]);
    if (!validation.valid) {
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-3 text-sm">
           <div className="bg-primary-50 text-primary px-2 py-1 rounded-md font-medium">
              You: {currentUserElo}
           </div>
           <div className="flex items-center space-x-1 text-text-secondary">
             <Icon name="Target" size={16} />
             <span>{skillRange[0]} - {skillRange[1]}</span>
           </div>
        </div>
      </div>
      ...
      <div className="space-y-3">
       ...
       {/* Error Message */}
       {error && (
        <div className="p-3 bg-error-50 border border-error-200 rounded-md flex items-start gap-2 animate-fade-in">
