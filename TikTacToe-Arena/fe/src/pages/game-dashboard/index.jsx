...
const GameDashboard = () => {
    ...
    const gamesWithProfiles = await Promise.all(
      games.map(async (game) => {
        ...
        if (!game.opponent.name || game.opponent.name === "Unknown Opponent") {
          ...
          let targetId = null;
          if (game.player1_id && game.player1_id !== user.id)
            targetId = game.player1_id;
          if (game.player2_id && game.player2_id !== user.id)
            targetId = game.player2_id;
  ...
  useEffect(() => {
    ...
    if (socket) {
      ...
      // If already connected, fetch data immediately to ensure sync
      if (socket.connected) {
        fetchData();
      }
    }
  ...
  const displayUser = {
    ...
    rank:
      (displayData?.current_rank || userProfile?.current_rank) === "Unranked"
        ? "Unranked"
        : `#${displayData?.current_rank || userProfile?.current_rank || "-"}`,
    eloChange: 0,
    status: "online",
  };
  ...
  return (
    <div className="min-h-screen bg-background">
      <GameContextHeader title="TicTacToe Arena" gameState="idle" />
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-3 space-y-6">
            {/* Welcome Card */}
            <WelcomeCard user={displayUser} onViewProfile={handleViewProfile} />
