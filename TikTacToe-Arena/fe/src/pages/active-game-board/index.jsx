...
const ActiveGameBoard = () => {
  ...
  useEffect(() => {
      const onConnect = () => {
          setIsConnected(true);
          // Refresh game state on reconnection to sync moves missed during disconnect
          if (gameId) { 
              socketService.emit('join_game', { game_id: gameId });
              // Trigger a fetch to get latest state including timer
          }
      };
  ...
  }, [gameId]); // added gameId dependancy so we join correct room on connect
  ...
  useEffect(() => {
    let interval;
    // Reverted: Start timer immediately when game is active, regardless of moves
    // const hasMoves = gameState.some(cell => cell !== null);
