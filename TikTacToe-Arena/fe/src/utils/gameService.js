...
const gameService = {
  ...
  leaveMatchmaking: async (userId) => {
    try {
      const MATCHMAKING_URL = import.meta.env.VITE_MATCHMAKING_SERVICE_URL || 'http://localhost:5003';
      const response = await axios.post(`${MATCHMAKING_URL}/queue/leave`, { user_id: userId });
      return { success: true, data: response.data };
    } catch (error) {
      console.error('leaveMatchmaking error:', error);
      return { success: false, error: 'Failed to leave matchmaking queue' };
    }
  },

  isInMatchmaking: async (userId) => {
    try {
      const MATCHMAKING_URL = import.meta.env.VITE_MATCHMAKING_SERVICE_URL || 'http://localhost:5003';
      const response = await axios.get(`${MATCHMAKING_URL}/queue/exists/${encodeURIComponent(userId)}`);
      return { success: true, data: response.data };
    } catch (error) {
      console.error('isInMatchmaking error:', error);
      return { success: false, error: 'Failed to check matchmaking status' };
    }
  },

  cancelGame: async (gameId, userId) => {
    try {
        const response = await gameClient.post(`/${gameId}/cancel`, { user_id: userId });
        return { success: true, data: response.data };
    } catch (error) {
