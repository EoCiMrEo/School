...
def get_recent_games(user_id):
    try:
        ...
        # Include 'completed' and 'invalid' games
        games = Game.query.filter(
            ((Game.player1_id == uid) | (Game.player2_id == uid)),
            Game.status.in_(['completed', 'invalid', 'cancelled'])
        ).order_by(Game.finished_at.desc()).offset(offset).limit(limit).all()
        return jsonify([g.to_dict() for g in games]), 200
    except ValueError:
        return jsonify({'error': 'Invalid user_id'}), 400

@game_bp.route('/games/<game_id>/cancel', methods=['POST'])
def cancel_game(game_id):
    ...
    try:
        ...
        if game.player1_id != uid and game.player2_id != uid:
             return jsonify({'error': 'User not in this game'}), 403
             
        # 3. Cancel Game
        # Only allow cancellation if game is not already completed
        if game.status == 'completed':
            return jsonify({'error': 'Game already completed'}), 400
            
        game.status = 'invalid' # As requested
        game.finished_at = datetime.utcnow()
        # No winner for cancellation
        db.session.commit()
        
        # Delete from Redis to invalidate cache
        try:
            r = get_redis_client()
            r.delete(f"game:{game_id}:state")
        except Exception as e:
            current_app.logger.warning(f"Failed to delete redis key for {game_id}: {e}")
        
        # 4. Notify Players via Redis
        publish_game_update(game_id, 'match_cancelled', {
            'game_id': game_id,
            'cancelled_by': user_id,
            'status': 'invalid'
        })
        
        # Notify dashboards to remove/update game
        publish_user_update(str(game.player1_id), 'dashboard_update', {'type': 'game_cancelled', 'game_id': game_id, 'status': 'invalid'})
        if game.player2_id:
            publish_user_update(str(game.player2_id), 'dashboard_update', {'type': 'game_cancelled', 'game_id': game_id, 'status': 'invalid'})

        return jsonify({'message': 'Game cancelled', 'game_id': game_id, 'status': 'invalid'}), 200

    except ValueError:
        return jsonify({'error': 'Invalid user_id'}), 400
    except Exception as e:
        current_app.logger.error(f"Error cancelling game {game_id}: {e}")
