...
def join_queue():
    ...
    # Validation: Ensure range includes user's Elo
    if not (min_elo <= elo <= max_elo):
         return jsonify({'error': 'Search range must include your current ELO'}), 400

    # Validation: Min allowed Elo based on Rank
    def get_min_allowed_match(user_elo):
        if user_elo < 1200: return 0      # Bronze
        if user_elo < 1500: return 800    # Silver
        if user_elo < 1800: return 1200   # Gold
        if user_elo < 2100: return 1500   # Platinum
        if user_elo < 2400: return 1800   # Diamond
        return 2100                       # Master+

    allowed_min = get_min_allowed_match(elo)
    if min_elo < allowed_min:
        return jsonify({'error': f'Your rank cannot search below {allowed_min} ELO'}), 400
    ...
    if existing:
        # Do not allow re-joining while already queued. Return conflict so clients
        # can show a clear message and avoid duplicate actions (client should
        # instead poll or show current queue status).
        return jsonify({'error': 'User already in queue', 'code': 'ALREADY_QUEUED'}), 409
...
@matchmaking_bp.route('/queue/exists/<user_id>', methods=['GET'])
def check_exists(user_id):
    """Return whether the given user is currently in the matchmaking queue."""
    if not user_id:
        return jsonify({'error': 'Missing user_id'}), 400

    entry = MatchQueue.query.filter_by(user_id=user_id).first()
    if entry:
        return jsonify({'in_queue': True, 'entry': entry.to_dict()}), 200
