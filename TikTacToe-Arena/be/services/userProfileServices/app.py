...
def create_app(config_name='default'):
    ...
    # Support multiple origins (comma-separated) for CORS with credentials
    frontend_urls = os.getenv('FRONTEND_URL', 'http://localhost:4028').split(',')
    frontend_urls = [url.strip() for url in frontend_urls]  # Remove any whitespace
    cors.init_app(app, resources={r"/*": {"origins": frontend_urls}}, supports_credentials=True)
